...
type HierarchicalSearchProvider struct {
	owner string
}

func NewHierarchicalSearchProvider(owner string) (*HierarchicalSearchProvider, error) {
	return &HierarchicalSearchProvider{owner: owner}, nil
}

func (p *HierarchicalSearchProvider) Search(storeName string, embeddingProviderName string, embeddingProviderObj embedding.EmbeddingProvider, modelProviderName string, text string, knowledgeCount int) ([]Vector, *embedding.EmbeddingResult, error) {
	...
	if candidateTitlesNum > 10 {
		candidateTitlesNum = 10
	}

	question, _, err := getEnhancedQuestionByModel(modelProviderName, text, titleCandidates, candidateTitlesNum)
	if err != nil {
		return nil, nil, err
	}
...
func getEnhancedQuestionByModel(modelProviderName string, text string, titleCandidates []string, candidateTitlesNum int) (string, *model.ModelResult, error) {
	prompt := fmt.Sprintf(`
	Please help me select the top %d titles that are most likely to contain the answer. Just return the title list. No other content.
	`, candidateTitlesNum)
